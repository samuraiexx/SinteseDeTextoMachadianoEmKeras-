...
f.write('Epoch\tIteration\tLoss\tValidation Loss\n')
f.close()

    
for epoch in range(0, total_epochs - 1):
    for it in range(0, saves_per_epoch - 1):
        history = model.fit(X[len(sentences)*it//saves_per_epoch:len(sentences)*(it+1)//saves_per_epoch], y[len(sentences)*it//saves_per_epoch:len(sentences)*(it+1)//saves_per_epoch], batch_size=128, nb_epoch=1, validation_split=0.2)
        plot_loss.append(history.history['loss'][0])
        plot_loss.append(history.history['val_loss'][0])
        file_name = train_name + '_Epoch-' + str(epoch) + '_Iteration-' + str(it) # Escreve o nome do arquivo a ser salvo
        ...
        f.write(str(epoch) + '\t' + str(it) + '\t' + str(history.history['loss'][0]) + '\t' + str(history.history['val_loss'][0]) + '\n')
        f.close()
